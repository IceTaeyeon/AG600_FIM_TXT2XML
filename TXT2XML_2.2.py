...
class FaultIsolationConverter:
    ...
    def parse_fault_procedures(self, content):
        ...
        for i in range(1, len(sections), 2):
            if i + 1 < len(sections):
                ...
                if procedure:
                    # 调试输出
                    print(f"故障描述: {procedure['fault_description'][:50]}...")
                    print(f"初步评估内容长度: {len(procedure['initial_evaluation'])} 字符")
                    print(f"可能原因数量: {len(procedure['possible_causes'])}")
                    print(f"隔离步骤数量: {len(procedure['isolation_steps'])}")
    ...
    def parse_isolation_steps(self, steps_text):
        ...
        for step_num, step_content in main_matches:
            ...
            # 提取主步骤标题
            # 首先尝试提取编号后、第一个子步骤前的内容作为标题
            title_match = re.match(r'^([^(]+?)(?:\([a-z]\)|$)', step_content, re.DOTALL)
            if title_match:
            ...
            # 如果标题为空或只有换行符，尝试从内容中提取
            if not title or title.isspace():
                if '检查' in step_content:
                    match = re.search(r'检查([^。，\n]+)', step_content)
                    title = f"检查{match.group(1)}。" if match else f"步骤{step_num}"
                elif '更换' in step_content:
                    match = re.search(r'更换([^。，\n]+)', step_content)
                    title = f"更换{match.group(1)}。" if match else f"步骤{step_num}"
                else:
                    title = f"步骤{step_num}"
    ...
    def parse_sub_steps(self, step_content):
        ...
        for sub_letter, sub_content in sub_matches:
            ...
            if wire_match:
                ...
                wires = [w.strip() for w in re.split(r'[、，]', wire_text) if w.strip()]
...
<dmCode assyCode="{ref_parts["assyCode"]}" disassyCode="{ref_parts["disassyCode"]}" disassyCodeVariant="{ref_parts["disassyCodeVariant"]}" infoCode="{ref_parts["infoCode"]}" infoCodeVariant="{ref_parts["infoCodeVariant"]}" itemLocationCode="{ref_parts["itemLocationCode"]}" modelIdentCode="{ref_parts["modelIdentCode"]}"
    ...
    def generate_xml(self, procedure):
        ...
        if fault_desc.startswith("OMS故障代码为") and "故障名称为" in fault_desc:
            # 情况 1：OMS 格式
            code_match = re.search(r'OMS故障代码为[“"”](.+?)[“"”]', fault_desc)
            name_match = re.search(r'故障名称为[“"](.+?)[“"”]', fault_desc)
            fault_code = code_match.group(1) if code_match else ""
            fault_name = name_match.group(1) if name_match else ""
            fault_name = fault_name.rstrip("。")
            description = f"本程序给出了故障代码为“{fault_code}（{fault_name}）”的故障隔离程序。"

        elif fault_desc.startswith("CAS告警信息为"):
            # 情况 2：CAS 格式
            match = re.search(r'CAS告警信息为[“"”](.+?)[“"”]', fault_desc)
            fault_info = match.group(1) if match else ""
            fault_info = fault_info.rstrip("。")
            description = f"本程序给出了CAS告警信息为“{fault_info}”的故障隔离程序。"

        else:
            # 情况 3：普通格式 （FMEA）
            name_match = re.search(r'故障名称为[“"”](.+?)[“"”]', fault_desc)
            fault_name = name_match.group(1) if name_match else "未知故障"
            fault_name = fault_name.rstrip("。")
        ...
        if procedure['initial_evaluation']:
            ...
            if re.search(r'\(\d+\)', initial_content):
                ...
                for step in eval_steps:
                    ...
                    # 如果标题只是"步骤X"，尝试从内容中提取更好的标题
                    if step_title.startswith("步骤") or re.match(r'^\(\d+\)', step_title):
                        # 从步骤内容中提取实际操作
                        step_pattern = rf'\({step["number"]}\)\s*([^(]+?)(?:\([a-z]\)|$)'
                        title_match = re.search(step_pattern, initial_content, re.DOTALL)
                        if title_match:
                            step_title = title_match.group(1).strip()
                            if not step_title.endswith('。'):
                                step_title += '。'

                    lines.append(f'<para>{step_title}</para>')
                    ...
                    if step['sub_steps']:
                        ...
                        # 如果没有子步骤，直接处理整个步骤内容
                        # 从原始内容中提取该步骤的完整内容
                        step_pattern = rf'\({step["number"]}\)\s*(.*?)(?=\(\d+\)|$)'
                        step_match = re.search(step_pattern, initial_content, re.DOTALL)
                        if step_match:
                            step_full_content = step_match.group(1).strip()

                            # 查找DMC编码
                            dmc_pattern = r'AG600-A-\d{2}-\d+-\d+-\d+A-\d+A-A'
                            dmc_codes = re.findall(dmc_pattern, step_full_content)
                            processed_content = step_full_content

                            for dmc_code in dmc_codes:
                                dm_ref_xml = self.create_dm_ref_xml(dmc_code)
                                processed_content = processed_content.replace(dmc_code, dm_ref_xml)
                ...
                # 没有编号步骤的情况，作为单个段落处理
                dmc_pattern = r'AG600-A-\d{2}-\d+-\d+-\d+A-\d+A-A'
                dmc_codes = re.findall(dmc_pattern, initial_content)
                processed_content = initial_content
